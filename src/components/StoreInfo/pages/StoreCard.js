...
import storeImage from './StoreListImage/store_example.svg'; // 카드에 사용할 이미지

// 부모 컴포넌트(StoreList)로부터 'store' 데이터를 props로 받습니다.
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden transform hover:-translate-y-2 transition-transform duration-300">
      ...
      <div className="p-6">
        ...
        <div className="flex gap-2">
          <span className="bg-gray-100 text-gray-700 text-xs font-medium px-3 py-1 rounded-full">{store.region}</span>
        </div>
      </div>
    </div>
  );
