  ...
  if (imageNames && imageNames.length > 0) {
    try {
      ...
      return limitedImages.map((imageName) =>
        require(`./RealStoreImage/${storeName}/${imageName}`)
      );
    } catch (error) {
...
const StoreModal = ({ store, onClose }) => {
  ...
  const defaultStore = {
    name: machineData?.name || '호텔런드리 암사점',
    address: machineData?.address?.value || '서울 강동구 올림픽로 98길 41 1층',
  ...
  const currentStore = store
    ? {
        ...
        name: machineData?.name || store.name,
        address: machineData?.address?.value || store.address,
        phone: getStorePhone(store.name), // 전화번호는 하드코딩된 매핑 사용
        tags: [store.region, '20평형'],
  ...
  if (loading) {
    return (
      <div
        className="fixed inset-0 z-50 overflow-y-auto font-pretendard"
      ...
      >
        <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
          <div
            ...
            onClick={onClose}
          ></div>
          <div className="inline-block align-bottom bg-white rounded-2xl text-center overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full p-8">
    ...
    >
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        ...
        <div className="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl w-full p-6 sm:p-8">
          <h2 className="text-[22px] sm:text-[24px] md:text-[28px] lg:text-[32px] xl:text-[36px] 2xl:text-[40px] font-bold text-[#1C262B] font-KoPubWorldDotum leading-normal tracking-[-0.48px] sm:tracking-[-0.48px] md:tracking-[-0.56px] lg:tracking-[-0.64px] xl:tracking-[-0.72px] 2xl:tracking-normal mb-[20px] sm:mb-[30px] md:mb-[20px] lg:mb-[30px] xl:mb-[30px] 2xl:mb-[30px] text-center">
            매장 상세 정보
          </h2>
          ...
          <div className="space-y-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                {' '}
                {/* md:grid-cols-3에서 md:grid-cols-2로 변경 */}
